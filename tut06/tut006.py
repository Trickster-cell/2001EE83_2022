...
os.makedirs('Outputtemp', exist_ok=True)
filepath = "Outputtemp/attendance_report_consolidated1.xlsx"
...
x = 3
for i in allValid:
    sheet.cell(row=1, column=x).value = allValid[x-3]
    x+=1 
sheet.cell(row=1, column=x).value = "Actual Lecture Taken"
x+=1
sheet.cell(row=1, column=x).value = "Total Real"
x+=1
sheet.cell(row=1, column=x).value = "% Attendance"
x+=1

y = 2
for i in range(len(Rolls)):
    sheet.cell(row = y, column = 1).value = Rolls[i]
    sheet.cell(row = y, column = 2).value = Names[i]
    y+=1

for i in range(len(Rolls)):
    ftemp = "Outputtemp/" + (str)(Rolls[i]) + ".xlsx"
    wbtemp = openpyxl.Workbook()
    wbtemp.save(ftemp)
    # made the xl files with name of rolls.
    
    headings = ["Date","Roll","Name","Total_Attendance_Count","Real","Duplicate","Invalid","Absent"]
    
    wbwork = load_workbook(ftemp)
    sheetwork = wbwork.active
    for j in range(len(headings)):
        sheetwork.cell(row=1, column=j+1).value = headings[j]
    
    sheetwork.cell(row=2, column=2).value = Rolls[j]
    sheetwork.cell(row=2, column=3).value = Names[j]
